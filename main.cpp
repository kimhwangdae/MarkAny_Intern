	...
	try
	{
		...
		do {
			...
			if (pressed_key >= 7340032 && pressed_key <= 8060928) {
				switch (pressed_key) {
				case 8060928:
					std::string imgpath = images_path + "/" + jpg_filenames[trackbar_value];
					std::string txtpath = images_path + "/" + txt_filenames[trackbar_value];

					int result_j = std::remove(imgpath.c_str());	// jpg 삭제
					int result_t = std::remove(txtpath.c_str());	// txt 삭제

					if (result_j == 0) {
						std::cout << jpg_filenames[trackbar_value] << " -> file remove!\n";
					}
					else {
						std::cout << jpg_filenames[trackbar_value] << " -> remove FAIL...\n";
					}

					if (result_t == 0) {
						std::cout << txt_filenames[trackbar_value] << " -> file remove!\n";
					}
					else {
						std::cout << txt_filenames[trackbar_value] << " -> remove FAIL...\n";
					}
					remove_flag = 1;
					break;
				}
